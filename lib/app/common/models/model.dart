...
class WidgetCategory {
  ...
  factory WidgetCategory.fromJson(Map<String, dynamic> json) {
    return WidgetCategory(json['categoryName'], json['controlList'], json['mobileCategoryId'],
        json['webCategoryId'], json['platformsToHide']);
  }
...
class Control {
  ...
  Control(this.title, this.description, this.image, this.status, this.displayType, this.subItems,
      this.controlId, this.isBeta, this.platformsToHide);
...
Future<void> updateControlItems() async {
            ...
            if (firstLevelSubItems[j].type == 'parent') {
              for (int k = 0; k < firstLevelSubItems[j].subItems!.length; k++) {
                if (SubItem.fromJson(firstLevelSubItems[j].subItems![k]).platformsToHide == null ||
                    _needToShow(
                        SubItem.fromJson(firstLevelSubItems[j].subItems![k]).platformsToHide)) {
                  secondLevelSubItems.add(SubItem.fromJson(firstLevelSubItems[j].subItems![k]));
                  for (int l = 0;
                      l < secondLevelSubItems[secondLevelSubItems.length - 1].subItems!.length;
                      l++) {
                    if (SubItem.fromJson(secondLevelSubItems[secondLevelSubItems.length - 1]
                                    .subItems![l])
                                .platformsToHide ==
                            null ||
                        _needToShow(SubItem.fromJson(
                                secondLevelSubItems[secondLevelSubItems.length - 1].subItems![l])
                            .platformsToHide)) {
                    ...
                    thirdLevelSubItems[thirdLevelSubItems.length - 1].sampleIndex ??=
                    ...
                    final String breadCrumbText =
                        ('/${controlList[i].title!}/${firstLevelSubItems[j].title!}/${secondLevelSubItems[secondLevelSubItems.length - 1].title!}${secondLevelSubItems[secondLevelSubItems.length - 1].subItems!.length == 1 ? '' : ('/${thirdLevelSubItems[thirdLevelSubItems.length - 1].title!}')}')
                            .replaceAll(' ', '-')
                            .toLowerCase();
                    thirdLevelSubItems[thirdLevelSubItems.length - 1].breadCrumbText =
                        breadCrumbText;
                    thirdLevelSubItems[thirdLevelSubItems.length - 1].categoryName =
                        SampleModel._categoryList[index].categoryName;
                    sampleRoutes.add(SampleRoute(
                        routeName: breadCrumbText,
                        subItem: thirdLevelSubItems[thirdLevelSubItems.length - 1]));
                  }
              ...
              if (firstLevelSubItems[j].platformsToHide == null ||
                ...
                isChild = true;
                for (int k = 0; k < firstLevelSubItems[j].subItems!.length; k++) {
                  if (SubItem.fromJson(firstLevelSubItems[j].subItems![k]).platformsToHide ==
                          null ||
                      _needToShow(
                          SubItem.fromJson(firstLevelSubItems[j].subItems![k]).platformsToHide)) {
                    secondLevelSubItems.add(SubItem.fromJson(firstLevelSubItems[j].subItems![k]));
                    ...
                    secondLevelSubItems[secondLevelSubItems.length - 1].breadCrumbText =
                        breadCrumbText;
                    secondLevelSubItems[secondLevelSubItems.length - 1].categoryName =
                        SampleModel._categoryList[index].categoryName;
                    sampleRoutes.add(SampleRoute(
                        routeName: breadCrumbText,
                        subItem: secondLevelSubItems[secondLevelSubItems.length - 1]));
                  }
                }
                firstLevelSubItems[j].subItems = secondLevelSubItems;
                secondLevelSubItems = <SubItem>[];
              ...
              if (firstLevelSubItems[j].platformsToHide == null ||
                ...
                sampleRoutes
                    .add(SampleRoute(routeName: breadCrumbText, subItem: firstLevelSubItems[j]));
  ...
  for (int i = 0; i < SampleModel._categoryList.length; i++) {
    SampleModel._categoryList[i].controlList!
        .sort((dynamic a, dynamic b) => a.controlId.compareTo(b.controlId) as int);
  }

  if (isWeb) {
    /// Sorting categories based on [webCategoryId]
    SampleModel._categoryList
        .sort((WidgetCategory a, WidgetCategory b) => a.webCategoryId!.compareTo(b.webCategoryId!));
  } else {
    /// Sorting categories based on [mobileCategoryId]
    SampleModel._categoryList.sort(
        (WidgetCategory a, WidgetCategory b) => a.mobileCategoryId!.compareTo(b.mobileCategoryId!));
  }
}
